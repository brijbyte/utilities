...
export function TotpGridSkeleton() {
  return (
    <div className="flex-1 p-pn-y px-pn-x overflow-auto flex flex-col gap-md w-full">
      <div className="grid gap-md grid-cols-1 md:grid-cols-2">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="bg-bg-surface border border-border p-md rounded-xl flex items-center gap-md relative overflow-hidden h-[74px]"
          >
